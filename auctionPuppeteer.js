...
async function dataScrape() {
    ...
    for (let auctionObject = 0; auctionObject <= auctionObjects.length - 1; auctionObject++) {
        ...
        for(let location of searchLocations){
            if (auctionObjects[auctionObject].title.includes(location) && (!auctionObjects[auctionObject].title.includes("HAS ENDED") &&(auctionObjects[auctionObject].title.includes(dateFilter)))) {
    ...
    for (let auction = 0; auction < auctions.length; auction++) {
        ...
        for (let i = 0; i < searchTerms.length; i++) {
            try {
                ...
                console.log(`Could not process the search ${searchTerms[i]}`)
